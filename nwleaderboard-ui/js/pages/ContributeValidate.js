...
export default function ContributeValidate() {
  ...
  const handleRunRemove = (runIndex) => {
    ...
    updateResult((current) => ({
      ...current,
      runs: Array.isArray(current.runs)
        ? current.runs.filter((_, currentIndex) => currentIndex !== runIndex)
        : [],
    }));
  };
              ...
              >
                ...
                <div className="contribute-file-runs">
                  {result.runs.map((run, runIndex) => {
                    ...
                    const modeValue = (run.mode || result?.context?.mode || '').toUpperCase();
                    const modeLabel =
                      modeValue === 'TIME'
